...
answer-cache backend. Fails open: if Redis is unreachable — or the cache is the
in-process demo backend, which has no atomic counter — we allow the request, so
a cache outage never takes the API down with it.
"""
...
async def retry_after(client_ip: str) -> int | None:
    ...
    # The in-process demo cache has no atomic incr/expire — fail open.
    if not hasattr(client, "incr"):
        return None
    now = int(time.time())
